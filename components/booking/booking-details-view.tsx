...
export function BookingDetailsView({ booking, academy, onBack, action }: BookingDetailsViewProps) {
    return (
        <div className="container mx-auto py-6 max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Header */}
            <div className="mb-6 flex flex-col gap-4">
                <div className="flex items-start gap-3">
                    <Button variant="ghost" size="icon" onClick={onBack} className="h-8 w-8 -ml-2 text-slate-500 hover:text-slate-800 cursor-pointer shrink-0 mt-1">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center justify-between gap-y-2">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                <h1 className="text-[16px] md:text-2xl font-bold text-[#271E4A] font-poppins wrap-break-word leading-tight">
                                    {booking.eventName || "Event Details"}
                                </h1>
                                <StatusBadge status={booking.status} />
                            </div>

                            {/* Optional Action Buttons (e.g. Approve/Reject) */}
                            {action && (
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500">
                            <span className="whitespace-nowrap">
                                Booking Id: <span className="font-mono text-[#7D3FD0]">#{booking.id.toUpperCase()}</span>
                            </span>
                            <span className="text-slate-300 hidden sm:inline">|</span>
                            <div className="flex items-center gap-1.5 whitespace-nowrap">
                                <Calendar className="h-4 w-4 text-slate-400" />
                                <span>{new Date(booking.date).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })} - {new Date(booking.date).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <CardContent className="pt-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-4 mb-2">
                                <div>
                                    ...
                                    <p className="font-medium text-slate-900 text-sm">{academy?.name || booking.academyId}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-slate-500 font-medium">Participants</p>
                                    <p className="font-medium text-slate-900 text-sm">{booking.participantsCount || "N/A"}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-slate-500 font-medium">Vertical</p>
                                    <p className="font-medium text-slate-900 text-sm">{booking.attendeesVertical || "N/A"}</p>
                                </div>
                                ...
                                <div className="md:col-span-1">
                                    ...
                                    <p className="font-medium text-slate-900 text-sm">{booking.merilianCode || "N/A"}</p>
                                </div>
                                <div className="md:col-span-1">
                                    <p className="text-xs text-slate-500 font-medium">Full Name</p>
                                    <p className="font-medium text-slate-900 text-sm">{booking.fullName || "N/A"}</p>
                                </div>
                                <div className="md:col-span-1">
                                    <p className="text-xs text-slate-500 font-medium">Contact Number</p>
                                    <p className="font-medium text-slate-900 text-sm">{booking.contactNumber || "N/A"}</p>
                                </div>
                                <div className="md:col-span-1">
                    ...
                    <Card className="shadow-sm border-slate-200">
                        ...
                        <CardContent className="pt-2 space-y-2">
                            <div>
                                ...
                                <span className="font-medium text-slate-900 text-sm">{booking.specificRequirements || "None specified"}</span>
                            </div>
                            <div>
                                <h4 className="text-xs text-slate-500 font-medium">IT Requirements</h4>
                                <span className="font-medium text-slate-900 text-sm">{booking.itRequirements || "None specified"}</span>
                            </div>
                            <div>
                                <h4 className="text-xs text-slate-500 font-medium">MATS Status</h4>
                                <span className={cn("font-medium text-slate-900 text-sm", booking.matsEvent === 'Yes' ? "text-green-600" : "text-slate-600")}>
                                    {booking.matsEvent || "No"}
                                </span>
                            </div>
                            <div>
                                <h4 className="text-xs text-slate-500 font-medium">MATS Request #</h4>
                                <span className="font-medium text-slate-900 text-sm">{booking.matsRequestNo || "None specified"}</span>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="lg:col-span-3 space-y-2">
                    <Card className="shadow-sm border-slate-200">
                        <CardHeader className="bg-slate-50/50 border-b border-slate-100 py-2">
                            <CardTitle className="text-[14px] md:text-lg font-medium text-slate-800">
                                Schedule - Daywise Plan ({booking.sessions?.length || 1} Days)
                            </CardTitle>
                        </CardHeader>
                                    ...
                                    <div key={idx} className="bg-white border border-slate-200 rounded-lg p-3 shadow-md transition-shadow group flex flex-col gap-3">
                                        ...
                                        <div className="space-y-1.5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-medium text-slate-700">{session.startTime || "09:00"} - {session.endTime || "17:00"}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <MapPin className="h-3.5 w-3.5 text-slate-400" />
                                                <span className="text-xs font-medium text-slate-700">{booking.hallId === 'h1' ? "Lister Hall" : "Hall 2"}</span>
                                            </div>
                                        </div>
                                    </div>
                                )) : (
